...
abstract class DB extends PDO
{
	 ...
	 * @return array
	 */
	public abstract function selectAsObjects(PDOStatement $stmnt, $className, array $params = array());
	 ...
	 * @return array
	 */
	public abstract function select(PDOStatement $stmnt, array $params = array());
	 ...
	 */
	public abstract function insert(PDOStatement $stmnt, array $params = array());
	 ...
	 */
	public abstract function update(PDOStatement $stmnt, array $params);
	 ...
	 * @return int Affected rows
	 */
	public abstract function delete(PDOStatement $stmnt, array $params);

	/**
	 * Returns the DBConnect object with the current used connection
	 * @return DBConnect
	 */
	public abstract function getDbConnect();
	...
	public function execute(PDOStatement $stmnt)
	{
		try {
			...
			$dbException = new DBQueryException('PDO could not execute query: ' . $e->getMessage(), $e->errorInfo[1], $stmnt->queryString);
	...
	public function beginTransaction($transactionName = null)
	{
		...
		try {
			...
			$dbException = new DBException('PDO could not begin transaction: ' . $e->getMessage(), $e->getCode());
			$dbException->errorInfo = $e->errorInfo;
	...
	public function commit()
	{
		try {
			...
			$dbException = new DBException('PDO could not commit transaction: ' . $e->getMessage(), $e->getCode());
			$dbException->errorInfo = $e->errorInfo;
